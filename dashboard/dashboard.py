...
engine = create_engine("postgresql://postgres:Mara3011*@localhost:5433/Evolve")
...
def calculate_metrics(df, user_id):
    ...
    avg_question_length = questions.str.len().mean() if not questions.empty else 0

    # Average response length
    avg_response_length = responses.str.len().mean() if not responses.empty else 0
    ...
    return {
        ...
        "response_to_question_ratio": round(response_to_question_ratio, 2)
    }
